...
def select(id):
    ...
    if result is not None:
        ...
        book = Book(result['title'], author, result['genre'], result['publisher'], result['available'], result['id'])
    return book
...
def update(book):
    sql = "UPDATE books SET (title, author_id, genre, publisher, available = (%s, %s, %s, %s, %s) WHERE id = %s"
    values = [book.title, book.author.id, book.genre, book.publisher, book.available]
    run_sql(sql, values)